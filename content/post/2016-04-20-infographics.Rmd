---
title: Creating an Infographic with infogram
author: Paul Oldham
date: '2016-04-20'
slug: infographics
categories:
  - Patents
  - R
tags: [infographics, infogram, patent analytics, R]
---
...
In this article we will use RStudio to prepare patent data for visualisation in an infographic using the online software tool [infogram](https://infogram.com/?rc=paid0sem0branded0search0&gclid=EAIaIQobChMIw6KgvMiq2AIViLvtCh2fpgxhEAAYASAAEgKR2PD_BwE). 
...
country_totals <- select(pizza_1990_2012, publication_country_name, publication_number, pubyear) %>%
  mutate(n = sum(publication_number = 1)) %>% 
  count(publication_country_name, pubyear, wt = n) %>%
  spread(publication_country_name, nn)
country_totals
```
...
{
  p_count <- dplyr::select(data, !!col, !!count_col) %>%
    tidyr::separate_rows(col, sep = sep) %>% 
    dplyr::mutate(!!col := stringr::str_trim(.[[col]], side = "both")) %>% 
    dplyr::mutate(n = sum(count_col = 1)) %>% 
    dplyr::group_by(!!col := .[[col]]) %>% 
...
```{r echo=FALSE}
knitr::include_graphics("/images/infogram/fig1_infogram_front.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("/images/infogram/fig2_infogram_login.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("/images/infogram/fig3_infogram_findedit.png")
```

This will open up a data panel with the toy data displayed. 

```{r echo=FALSE}
knitr::include_graphics("images/infogram/fig4_infogram_datapanel.png")
```

We want to replace this data by choosing the upload button and selecting our `pizza_country_1990_2012.csv` file. 

```{r echo=FALSE}
knitr::include_graphics("/images/infogram/fig5_infogram_panelgraph.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("/images/infogram/fig6_infogram_paneltext.png")
```
